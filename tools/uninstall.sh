#!/usr/bin/env bash
# tools/uninstall.sh — Reverse a cognitive-claude install cleanly.
#
# Idempotent. Read-only by default; --apply to actually remove.
# Restores from ~/.claude/.backups/ if present.
#
# Usage:
#   bash tools/uninstall.sh                     # dry-run (default)
#   bash tools/uninstall.sh --apply             # remove + restore from backup
#   bash tools/uninstall.sh --apply --keep-telemetry
#                                               # remove hooks, keep telemetry logs
...
if [ -n "$LATEST_SETTINGS" ]; then
  ...
  log "         search for: bash ~/.claude/hooks/{telemetry,cache-guard,token-economy-*,tier-contradiction-guard}.sh"
fi
...
# 3. Remove hooks
log "--- removing hooks ---"
for h in telemetry.sh \
         cache-guard.sh \
         token-economy-guard.sh \
         tier-contradiction-guard.sh \
         token-economy-boot.sh \
         token-economy-session-end.sh; do
  path="${CLAUDE_DIR}/hooks/${h}"
  if [ -f "$path" ]; then
    if $APPLY; then
      rm "$path" && log "removed: $path"
    else
...
# 4. Telemetry directory — moved aside, never deleted by this script
...
if [ -d "$TELE_DIR" ]; then
  if $KEEP_TELEMETRY; then
    ...
    log "moved telemetry/ to ${TELE_DIR}.removed.<ts> (not deleted; remove manually if desired)"
  else
    log "would move: $TELE_DIR -> ${TELE_DIR}.removed.<ts>"
  fi
fi

# 5. cognitive-claude runtime artifacts (audit-history, bridge-history, etc)
CC_RUNTIME="${CLAUDE_DIR}/cognitive-claude"
if [ -d "$CC_RUNTIME" ]; then
  if $APPLY; then
    mv "$CC_RUNTIME" "${CC_RUNTIME}.removed.$(date +%Y%m%d-%H%M%S)"
    log "moved cognitive-claude/ runtime to ${CC_RUNTIME}.removed.<ts>"
  else
    log "would move: $CC_RUNTIME -> ${CC_RUNTIME}.removed.<ts>"
  fi
fi
...
if $APPLY; then
  ...
  log "DRY RUN. Nothing was changed. Re-run with --apply to execute."
fi
