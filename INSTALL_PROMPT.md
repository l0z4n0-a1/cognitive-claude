...
```bash
# Cache hit rate over last 14 days
find ~/.claude/projects -name "*.jsonl" -mtime -14 | xargs cat 2>/dev/null | python3 -c "
import sys, json
cr = inp = co = 0
turns = 0
...
total_input = cr + inp
print(f'Last 14 days, cognitive-claude active:')
print(f'  Turns analyzed:    {turns:,}')
print(f'  Cache hit rate:    {100*cr/total_input:.1f}%' if total_input else 'N/A')
print(f'  Total input:       {total_input/1e6:.1f}M tokens')
print(f'  Total output:      {co/1e6:.1f}M tokens')
print(f'  Cache reads:       {cr/1e6:.1f}M tokens')
"
```

Then explain the numbers in plain language. Tell them what the targets
are (cache hit ≥90%, etc.) and where they stand.

If they have not run cognitive-claude long enough, say so honestly.
