...
Sub-agent turn-share       ≥ 50% (turn-basis)   Lower OK (30–40%) — Sonnet
                                                ...
                                                cost is less penal.
Cost/turn target           < $0.50 (Opus heavy) Lower naturally (~$0.10).
                           API-equivalent       You'll hit target easily.
Plan-vs-API ratio          84× in case study    Smaller magnitude (typically
                           (heavy-Opus, ~$500   20–35× for Sonnet-default;
                           paid over 3 months)  same shape, less stark).
...
**Adaptation summary:** Install everything. Expect your plan-vs-API
ratio to be smaller in absolute magnitude (Sonnet is ~5× cheaper than
Opus per token) but the shape — measurement discipline, attribution,
defensibility — is identical.
