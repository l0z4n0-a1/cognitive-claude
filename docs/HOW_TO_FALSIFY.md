...
prerequisite: docs/CONFOUNDS.md, tools/cost-audit.py
...
**Claim:** A 5,000-token CLAUDE.md across ~150 turns processes ~750,000
tokens (`docs/MATH.md` §3). Use your own median turns/session from
`tools/cost-audit.py --verbose` to size for your workload.

**Falsification test:**
1. Run `python3 tools/cost-audit.py --json --window 14 > before.json`
...
- **`docs/LIMITATIONS.md`** §"What would falsify each claim" — the
  declarative version of these protocols. LIMITATIONS states *what*
  would falsify; this document operationalizes *how*.
- **`docs/CONFOUNDS.md`** — the post-hoc honesty pass that admits
  what the per-invariant tests below cannot prove in aggregate.
- **`docs/INVARIANTS.md`** §5 — the falsifiability matrix in table
  form, mapping each Law to its falsification test in this document.
- **`docs/REPLICATION_LOG.md`** — where to publish your results so
  the architecture is improved by being challenged from data.
