...
This document was written **after** the 189x measurement, in response to
...
> architecture. The 189x leverage cited at the top is overwhelmingly the
...
> — not 189x, but in the same order of magnitude. The architecture-multiplier
...
1. **Architecture-multiplier exists.** Cache hit rate of 92.4%+ sustained
   ...
   sub-agent ratio at 74.2% is consistent with the seven invariants being
...
1. **189x as architecture-only leverage.** It is plan × architecture.
...
3. **Causation over correlation.** The repo observes the 92.4% cache
...
The arithmetic temptation is to say "189x = plan_leverage × architecture_leverage,
plan_leverage ≈ 25-35x for heavy Max users, therefore architecture ≈ 5-8x."
But this division is **circular**: it uses the same 189x measurement to
derive both factors, then claims they were independently estimated.

What we can say honestly:

1. Each individual mechanism (M1: tax, M2: cache stability, M3: sub-agent
   isolation) has a derivable per-turn cost in tokens. See `docs/MATH.md`
   §3a, §6, §7. Those derivations don't depend on 189x.
2. Each individual discipline (D1-D4) is independently testable via
   `docs/HOW_TO_FALSIFY.md`. Those tests don't depend on 189x.
...
gains exist *at all*, even if the aggregate "189x" is misleading? Read
`docs/MATH.md` §6 (cache breaks) and §7 (sub-agent) and decide.

If you are about to cite the repo: cite the per-mechanism gains
(falsifiable, derivable), not the 189x combined figure. Anything else is
