...
**Implication:** A 5,000-token CLAUDE.md across 159 turns processes
795,000 tokens per session. A 1,300-token CLAUDE.md across the same
session processes 207,000. The difference is not 3.85x cost — it is
3.85x cost *plus* a smaller, more cacheable prefix that survives
longer.
...
Constitution × 159 turns × 5 sessions/day × 30 days = 119M tokens
of overhead per month. A 1.3k-token Constitution = 31M.
...
**Pricing implication:** Haiku is ~12x cheaper than Opus per token.
If 40% of your agent calls are pure retrieval (file search,
summarization), routing them to Haiku saves roughly 90% on those
calls. In a heavy-Opus workload, this is the second-largest lever
after MCP discipline.
...
frequency. The L0–L5 model makes this explicit.

```
   ┌────────────────────────────────────────────────────────────┐
   │  L0 — IMMUTABLE                                            │
   │      Harness defaults set by Anthropic. Never change.      │
   ├────────────────────────────────────────────────────────────┤
   │  L1 — CONSTITUTION         (~/.claude/CLAUDE.md)           │
   │      Operator-approved. Changes per quarter, not per day.  │
   ├────────────────────────────────────────────────────────────┤
   │  L2 — POLICY               (rules, hooks, permissions)     │
   │      Changes per quarter. Audited.                          │
   ├────────────────────────────────────────────────────────────┤
   │  L3 — CAPABILITY           (skills, agents)                 │
   │      Changes per month. Adds and archives.                 │
   ├────────────────────────────────────────────────────────────┤
   │  L4 — SESSION              (project CLAUDE.md)              │
   │      Changes per project. Extends L1, never contradicts.   │
   ├────────────────────────────────────────────────────────────┤
   │  L5 — RUNTIME              (telemetry, in-memory state)    │
   │      Changes every session. Read-only after capture.        │
   └────────────────────────────────────────────────────────────┘
```

**Rules:**

- A change to layer N can invalidate layers N+1 through 5.
- The reverse is forbidden — L4 cannot override L1.
- Mid-session changes to L1 or L2 break cache. Batch them to
  pre-session windows.
- L5 is data, not rules. Read it. Never edit it.
